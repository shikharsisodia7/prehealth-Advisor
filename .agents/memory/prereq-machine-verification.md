...
Batch extraction (2026-08): the review-queue root/cached page is often just the program homepage without the prereq list — extraction returning hasPrereqList=false usually means "wrong page", not "unpublished". Chase the dedicated prereq/admission-requirements subpage from candidateOfficialSources or a targeted web search (site:<domain> + "prerequisite courses") before concluding anything. A suspiciously short list (1–3 items) is the same signal — a checksheet PDF or dedicated page usually has the full list. Expansion-campus rows of the same institution can share the parent program's source. Generic upsert (`upsert-prereq-batch.ts`) reads a batch JSON and does the verified-write + read-back assertion.
