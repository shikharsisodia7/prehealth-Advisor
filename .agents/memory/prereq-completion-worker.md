...
description: How complete:prereqs loads secrets, uses Firecrawl/Jina/Keenable, and survives 402
---
...
Optional: `FIRECRAWL_API_KEY` (PDFs, 403, JS shells, search), `JINA_API_KEY` (r.jina.ai reader), `KEENABLE_API_KEY` (search + live page fetch), `COMPLETION_MODEL`, `COMPLETION_CONCURRENCY`.

**Firecrawl:** if the key is present, use it. HTTP 401/402 disables Firecrawl for the rest of that process and continues over Keenable / Jina / HTTP / Wikidata / DuckDuckGo / Bing. A 402 account (remaining credits −1) does not speed the queue until the plan is topped up. Do not strip `FIRECRAWL_API_KEY` from the environment when starting the worker unless you intend an HTTP-only run.

**Keenable / Jina:** preferred unpaid retrieval when Firecrawl is 402. Keenable search runs before Google/Bing/DDG. Keenable `live=true` fetch and Jina Reader run on 403/thin pages. Do not pass Keenable's `prompt` parameter — OpenAI must extract from the official page text, not a second model's summary.
